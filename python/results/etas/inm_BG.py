...
#----------------------------------------------------------------------
	...
	datafile1 = "iCS_mares.dat"
	Z, A, B, G = reading(datafile1, 5)
	
	datafile2 = "iCS_ours.dat"
...
# Plot----------------------------------------------------------------	
	...
	ax.plot(AA, BB, '--', markersize=15, linewidth=0.5, label=r'In-med CS ($\sqrt{s}$ Mares et. al.)')
	ax.plot(AA2, BB2, '--', markersize=15, linewidth=0.5, label=r'In-med CS ($\sqrt{s}$ ours)')
	for i in range(len(A)):
	...
	plt.text(AA[4]+xstep, BB2[4]+ystep-0.5, r'Pb')
	...
	ax2.plot(AA, GG, '--', markersize=15, linewidth=0.5, label=r'In-med CS ($\sqrt{s}$ Mares et. al.)')
	ax2.plot(AA2, GG2, '--', markersize=15, linewidth=0.5, label=r'In-med CS ($\sqrt{s}$ ours)')
	for i in range(len(A)):
